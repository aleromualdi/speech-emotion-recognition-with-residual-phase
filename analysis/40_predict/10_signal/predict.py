'''
In this script I measure the performance of the MFCCs-based CNN model on the
test set.
'''
...
"""
x_valcnn = np.loadtxt(
    '../../30_train_cnn_model/10_signal/output/x_valcnn.csv', delimiter=',')
y_val = np.loadtxt(
    '../../30_train_cnn_model/10_signal/output/y_val.csv', delimiter=',')
...
"""
json_file = open('../../30_train_cnn_model/10_signal/output/model.json', 'r')
...
loaded_model.load_weights(
    "../../30_train_cnn_model/10_signal/output/model_checkpoint.h5")
...
for i in tqdm(range(len(test_df))):
    ...
    mfccs_vec.append(mfccs)

df_mfccs = pd.DataFrame(mfccs_vec, test_df.label).reset_index()#.fillna(0)
ntest_df = pd.DataFrame(mfccs_vec).values
